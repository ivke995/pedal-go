...
- `lib/` — shared utilities, domain types, static featured-bike display data, pricing logic, domain services, and database boundary.
  ...
  - `lib/admin-dashboard/` contains server-only admin operations summary and reservation listing/search queries for protected dashboard routes.
...
- `/admin` renders database-backed operations summary cards from reservation, payment, inventory, bike-type, and availability-block query boundaries. `/admin/reservations` renders a database-backed list with reservation/customer search, reservation status filtering, payment status filtering, and payment state visibility; remaining sibling admin routes currently establish protected MVP section boundaries for pricing, availability, calendar, and reports.
